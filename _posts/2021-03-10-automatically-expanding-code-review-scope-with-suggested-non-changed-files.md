...
There has been research similar to this, including [[future-defective]{.ul}](https://rebels.cs.uwaterloo.ca/papers/msr2015_thongtanunam.pdf) files and [[Who Should Review My Code? A File Location-Based Code-Reviewer Recommendation Approach for Modern Code Review]{.ul}](https://naist-se.github.io/pastpublications/naistse-1154.pdf). What we need is the ability to find out what files are normally changed with other files, then flag those if they aren't changed as part of a PR. This article [[Temporal coupling in Git repositories]{.ul}](https://codescene.com/blog/software-revolution-part3/) shows how commit history can reveal file paths that tend to change together over time.

While that article is focused around avoiding [[Shotgun Surgery]{.ul}](https://martinfowler.com/articles/modularizing-react-apps.html#the-shotgun-surgery-problem) (finding classes which are correlated together too much) it can also be used to find which files should be modified together. Sometimes it's not possible to fully decouple files (e.g. when adding a translation string when modifying the UI.)
